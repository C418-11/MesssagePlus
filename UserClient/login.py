...
class LoginAuthenticationSystem:
    ...
    @showException
    def on_login(self, client, log_head):  # todo
        if client is None:
    ...
    def _login(self) -> None:  # todo
        ...
        def log_wrapper():
            def func() -> None:
                nonlocal client
                client = self.on_login(client, log_head)

            return func

        self.login_ui.setupUi()
        self.main_window.show()

        self.register_ui.RegisterButton.clicked.connect(log_wrapper())
    ...
    @showException
    def on_register(self, client, log_head, *_, **__):
        if client is None:
            password = self.register_ui.GetPassword.text()
            password: str
            if len(password) < 8:
    ...
    def register(self) -> None:
        ...
        def reg_wrapper():
            def func() -> None:
                nonlocal client
                client = self.on_register(client, log_head)
